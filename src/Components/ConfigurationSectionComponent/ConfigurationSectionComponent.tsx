...
const ConfigurationSectionComponent : React.FC <CanvusProps> = ({fabricCanvasRef}) => {
  return (
    <div className="flex flex-col gap-4 max-h-full overflow-y-auto ">
      ...
      <div className="flex ">
        <button   onClick={() => downloadCanvasAsPdf(fabricCanvasRef.current)} className="w-full py-2 text-xs bg-blue-700 text-blue-50 text-blue-50 border border-border-color focus:outline-none hover:bg-card-color flex items-center justify-between gap-2">
